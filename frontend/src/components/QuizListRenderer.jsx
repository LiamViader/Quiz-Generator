...
function QuizListRenderer({quizList, onQuizClick, onUnsolve}){
    return(
        <div style={{display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-start', alignContent: 'flex-start', height: '100%', width: '90%', marginLeft:'auto', marginRight:'auto'}}>
            {quizList.map((quiz, index) => (
                ...
                <QuizClosed quiz={quiz} onQuizClick={onQuizClick} key={quiz.loading ? quiz.tempId : quiz.quiz._id } onUnsolve={onUnsolve}/>            
            ))}
        </div>
    );
}
